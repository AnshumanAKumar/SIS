...
<option value="FOURTH">FOURTH</option>
        ...
        <br>
        <input type="submit" name="submit" value="SEARCH & PROCEED" id='myButton' />
</form>
</body>
</html>
<?php
...
if(isset($_POST['submit']))
{
    ...
    $branch=$_POST['branch'];
     ...
     $query = "SELECT * FROM `bhup` WHERE rn='$rn'
and branch='$branch' and year='$year'";
	$result = mysqli_query($conn,$query) or die(mysql_error());
	$rows = mysqli_num_rows($result);
        if($rows==1)
        {
	    $_SESSION['rn'] = $rn;
            echo "<div class='font'> <font color = 'white' <br>Record exists </font> 
            
            <br><a href='update.php' id='myButton'>PROCEED TO UPDATE</a></div>";
        } 
 else {
        echo "<div class='font'> <font color = 'white' RECORD does not exist </font> </div>";
